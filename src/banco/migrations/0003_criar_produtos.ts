...
export async function up(knex: Knex) {
  return knex.schema
    .createTable(ETableNames.produtos, (table) => {
      ...
      table.unique(['sh_sku', 'empresa_id'], { indexName: 'idx_unique_produto_shSku_empresaId' });
      table.unique(['p4m_sku', 'empresa_id'], { indexName: 'idx_unique_produto_p4mSku_empresaId' });
    })
    .then(() => {
